...
const source = await readFile(resolve(process.cwd(), "contracts/ShelfNote.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "ShelfNote.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["ShelfNote.sol"]?.ShelfNote;
...
console.log(`Deploying ShelfNote from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_SHELF_NOTE_CONTRACT_ADDRESS to this contract address in Vercel.");
